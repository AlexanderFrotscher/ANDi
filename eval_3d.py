...
def main():
    ...
    threshold_test = [round(x, 3) for x in np.arange(1.0, 2, 0.1)]
   
    dice_scores_mask = {i: [] for i in threshold_test}
    for i, (image, label) in enumerate(pbar):
        image = (image*2)-1
        num_steps=1000
        my_masks = torch.zeros(
                (
                    image.shape[0],
                    len(threshold_test),
                    128,
        ...
        for j in range(image.shape[4]):
            ...
            for k, key in enumerate(dice_scores_mask):
                mask = create_mask_2(zs, key, steps=num_steps, images=image[:,:,:,:,j])
                my_masks[:,k,:,:,j] = mask
        for j, key in enumerate(dice_scores_mask):
            dice_scores_mask[key].extend([float(x) for x in dice(my_masks[:,j,:,:,:], label)])
    ...
    df_mask.to_csv(
        "/mnt/lustre/baumgartner/bkc035/data/BraTS2021/mask_one_3D.csv"
    )
...
def create_mask_2(zs, th, steps, images):
    ...
    my_mean[my_mean != 0] = 1
    my_mean = F.interpolate(my_mean[None, :, :, :], mode="bilinear", size=(128, 128))
