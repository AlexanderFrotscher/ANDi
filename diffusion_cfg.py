...
class Diffusion:
    ...
    def dpm_encoder(self, model, images, timestemp=None):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(reversed(range(1, timestemp - 1)), position=0):
                ...
                # normal implementation does not work for our purpose. Using no noise is better
                var = (beta * (1 - alpha_hat_minus_one) / (1 - alpha_hat))  # this is option 2 for sigma**2
                noise = torch.rand_like(images)
                xts[:, i] = mean #+ var * noise
            xts[:, 0] = images
    ...
    def skip_differences(self, model, images, start = 100, stop=None, skip=25, pyramid = False):
        if stop is None:
            stop = self.noise_steps
        if start == 0:
            start = 1
