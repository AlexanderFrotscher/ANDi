...
#SBATCH --mem=650GB                 # Memory pool for all cores (see also --mem-per-cpu)
...
# Run our code
echo "-------- PYTHON OUTPUT ----------"
srun -n 1 -N 1 -c 16 --mem=650GB --partition=a100 --time=1-14:00 --gres=gpu:1 accelerate launch diffusion_cfg.py
echo "---------------------------------"
